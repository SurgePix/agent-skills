...
| `surgepix-generate-photo-poetic-editorial` | "极简诗意风格", "极简线条", "改成极简诗意", "极简诗意编辑插画", "poetic editorial" | Photo → locked **bold-line geometric poster** (not busy sketch) PNG |
...
>
> **Photo → illustration:** 「帮我把这张图改成超现实主义风格」→ `surgepix-generate-restaged-cinematic` (conceptual restage: metaphor + color-field set + surreal device — **not** flatten the backdrop). 「改成极简诗意风格」/「极简线条」→ `surgepix-generate-photo-poetic-editorial` (bold few strokes + geometric primitives + large negative space — **not** a busy grainy sketch). If the user only says「把这张照片做成插画」without style, ask.
