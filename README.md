...
| `surgepix-generate-photo-poetic-editorial` | "极简诗意风格", "极简线条", "改成极简诗意", "极简诗意编辑插画", "poetic editorial" | Photo → locked **risograph silhouette print** (not blob-logo poster) PNG |
...
>
> **Photo → illustration:** 「帮我把这张图改成超现实主义风格」→ `surgepix-generate-restaged-cinematic` (conceptual restage: metaphor + color-field set + surreal device — **not** flatten the backdrop). 「改成极简诗意风格」/「极简线条」→ `surgepix-generate-photo-poetic-editorial` (risograph silhouettes + railing line structure — **not** a blob-logo poster with color chips). If the user only says「把这张照片做成插画」without style, ask.
