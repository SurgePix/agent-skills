...
>
> **Photo → illustration:** 「帮我把这张图改成超现实主义风格」→ `surgepix-generate-restaged-cinematic` (conceptual restage: metaphor + color-field set + surreal device — **not** flatten the backdrop). 「改成极简诗意风格」→ `surgepix-generate-photo-poetic-editorial`. If the user only says「把这张照片做成插画」without style, ask.
