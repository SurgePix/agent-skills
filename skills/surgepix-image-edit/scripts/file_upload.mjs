 ...
 *   SURGEPIX_BASE_URL      (required) API base URL from .env or shell
...
async function main() {
  ...
  if (!filePath || filePath === "-h" || filePath === "--help") {
    ...
    console.error("");
    console.error("Required env: SURGEPIX_API_KEY, SURGEPIX_BASE_URL");
  ...
  if (!config.baseUrl) {
    fail(
      "SURGEPIX_BASE_URL not found. Set it in .env or the shell (see surgepix-setup skill).",
    );
  }
