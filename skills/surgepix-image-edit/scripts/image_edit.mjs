 ...
 *
 * 流程:
 *   1. 本地参考图 → 上传拿到 URL（必填至少一张；URL 则跳过上传）
 *   2. POST /tasks/image-edit（始终异步提交：API noWait=true）
 *   3. 根据 --nowait 决定是否本地轮询 GET /tasks/{taskId}
 *
 * 用法:
 *   node image_edit.mjs --prompt <text> --size <WxH>
...
async function imageEdit(options) {
  ...
  console.error(`[image-edit] refs=${reference.length} size=${size[0]}x${size[1]}`);
  return apiRequest("POST", "/tasks/image-edit", { body });
}
...
function printHelp() {
  console.error("Usage: node image_edit.mjs --prompt <text> --size <WxH> \\");
  console.error("         --reference <path-or-url> [--reference ...] \\");
  console.error("         [--session-id <id>] [--nowait <true|false>]");
  console.error("");
  console.error("  --prompt <text>           编辑指令（必填）");
  console.error("  --size <WxH>              目标宽高，例如 1024x1024（必填）");
  console.error("  --reference <path-or-url> 参考图（必填，可重复；本地路径自动上传）");
...
async function main() {
  ...
  if (reference.length < 1) {
    fail("缺少必填参数: 至少一个 --reference（本地路径或图片 URL）");
  }
...
export { imageEdit, pollUntilDone, resolveReference, parseSize };
