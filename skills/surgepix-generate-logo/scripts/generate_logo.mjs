#!/usr/bin/env node
/**
 * SurgePix 生成品牌 Logo CLI
 *
 * 流程:
 *   1. 本地参考图 → 上传拿到 URL（可选，直接传 URL 则跳过）
 *   2. POST /tasks/generate-logo（始终异步提交：API noWait=true，立即返回 taskId）
 *   3. 根据 --nowait 决定行为:
 *      - --nowait false（默认，同步）：脚本内部轮询 GET /tasks/{taskId} 直到 succeeded / failed
 *      - --nowait true（异步）：脚本立即返回 taskId，由 Agent 用 query-task 查询
 *
 * 用法:
 *   node generate_logo.mjs --brand-name <text>
 *                          [--prompt <text>] [--size <1024x1024>]
 *                          [--reference <path-or-url> ...] [--transparent <true|false>]
...
async function generateLogo(options) {
  const { brandName, prompt, size, reference, transparent, sessionId } = options;
  // API 始终异步提交，由脚本侧按 --nowait 决定是否轮询
  const body = {
    noWait: true,
    source: "skill",
    brandName,
  ...
  console.error(`[generate-logo] brandName=${brandName}`);
  return apiRequest("POST", "/tasks/generate-logo", { body });
}
...
function parseArgs() {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    } else if (arg === "--brand-name" && i + 1 < args.length) {
    ...
    } else if (arg === "--transparent" && i + 1 < args.length) {
      const next = args[++i];
      if (next !== "true" && next !== "false") {
        fail(`--transparent 必须是 true 或 false，收到: ${next}`);
      }
      parsed.transparent = next === "true";
    } else if (arg === "--session-id" && i + 1 < args.length) {
      parsed.sessionId = Number(args[++i]);
    } else if (arg === "-h" || arg === "--help") {
      console.error("Usage: node generate_logo.mjs --brand-name <text> \\");
      console.error("         [--prompt <text>] [--size <1024x1024|1024x1536|1536x1024>] \\");
      console.error("         [--reference <path-or-url> ...] [--transparent <true|false>] \\");
      console.error("         [--session-id <id>] [--nowait <true|false>]");
      console.error("");
      console.error("  --brand-name <text>       品牌或产品名（必填）");
      console.error("  --prompt <text>           补充设计需求（配色/风格等）");
      console.error("  --size <WxH>              默认 1024x1024；合法 1024x1024 / 1024x1536 / 1536x1024");
      console.error("  --reference <path-or-url> 参考图（本地路径自动上传，可重复）");
      console.error("  --transparent <bool>      是否透明底，默认 true");
...
async function main() {
  const { brandName, prompt, size, reference, transparent, sessionId, nowait } =
    parseArgs();
  ...
  if (!brandName) {
    fail("缺少必填参数: --brand-name");
  }
  if (size != null && !ALLOWED_SIZES.has(size)) {
    fail(
      `--size 非法: ${size}；合法值为 1024x1024 / 1024x1536 / 1536x1024`,
    );
  }
...
export { generateLogo, pollUntilDone, resolveReference };
